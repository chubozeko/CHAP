...
if($email != "" AND $pass != "") {
  if ( !filter_var($email,FILTER_VALIDATE_EMAIL) ) {
    ...
    $USER_INFO = mysqli_fetch_array(mysqli_query($con, "SELECT userid,email,password,usrname,usrsurname FROM user_info WHERE email='$email'"));
    $CHECK_EMAIL = $USER_INFO['email'];
    $CHECK_PASS = $USER_INFO['password'];
    $NAME = $USER_INFO['usrname'];
    $SURNAME = $USER_INFO['usrsurname'];
    ...
    if($email == $CHECK_EMAIL AND $password == $CHECK_PASS) {
      ...
      $_SESSION['user_id'] = $TAKE_ID;
      $_SESSION['user_ip'] = $IP;
      $_SESSION['email'] = $CHECK_EMAIL;
      $CSESSION = $_SESSION;

      # HERE WE NEED TO SET FETCHED USER_INFO DATA AS A SESSION VARIABLE
      ...
      if($control_Con) {
        ...
        $response->message = "Your Login success";
        $response->userID = $TAKE_ID;
        ...
        # $response->message = "Ooops... Something Went Wrong! Please Check your entered Data or Check Your Internet Connection.";
        $response->message = "Your Login Email or Password is invalid";
      }
    }
      ...
      $response->message = "Invaild User Information! Please Re-Check your Data.";
    }
  }
  ...
  $response->message = "Your Login Email or Password cannot be left Empty!";
}
